...
describe User do
  ...
  #end
  #
  it "should sync postcard subjects properly" do
    user = create(:greg_user)
    app = App.babygrams

    subject_1 = { name: "Alf", birthday: "2013-10-12 11:40:16 -0700", gender: "boy", subject_type: "baby" }
    subject_2 = { name: "Fuzz", birthday: "2012-10-01 11:40:16 -0700", gender: "boy", subject_type: "child" }

    user.set_postcard_subjects([subject_1, subject_2], app: app)

    user.reload
    user.postcard_subjects.size.should == 2
    alf = user.postcard_subjects.where(name: "Alf").first

    fuzz = user.postcard_subjects.where(name: "Fuzz").first
    fuzz.state.should == :active

    user.set_postcard_subjects([subject_1, subject_2, subject_1, subject_2], app: app)

    user.reload
    user.postcard_subjects.size.should == 2

    subject_1_change = { name: "Alf", birthday: "2013-10-14 11:40:16 -0700", gender: "boy", subject_type: "baby" }
    user.set_postcard_subjects([subject_2, subject_1_change], app: app)

    user.reload
    user.postcard_subjects.size.should == 3
    alf.reload
    alf.state.should == :removed

    alf = user.postcard_subjects.where(name: "Alf").select { |s| s.state == :active }.first
    alf.state.should == :active
